...
mod test {
    ...
    pub async fn test_max_retry_count() {
        ...
        let consumer_event01 = timeout(Duration::from_secs(5), rx.recv())
            .await
            .expect("should receive an event")
            .expect("should contain an event");
        ...
        let consumer_event02 = timeout(Duration::from_secs(5), rx.recv())
            .await
            .expect("should receive an event")
            .expect("should contain an event");
        ...
        let consumer_event03 = timeout(Duration::from_secs(5), rx.recv())
            .await
            .expect("should receive an event")
            .expect("should contain an event");
    ...
    pub async fn test_success_task() {
        ...
        // check success event
        let success_handle = spawn_check_handler(
            task_consumer.clone(),
            |event| match event {
                ConsumerEvent::MarkSuccess { key } => key == "111",
                _ => false,
            },
            Duration::from_secs(5),
        );
        // send task
        let param = TestConsumeParam::builder()
            .timeout_sec(1_u32)
            .emit_error(false)
            .build();
        task_producer
            .send_task("111", param, None)
            .await
            .expect("failed to send task");

        // wait for task to consumed and success
        assert!(
            success_handle.await.expect("failed to wait").is_some(),

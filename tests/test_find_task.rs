...
mod test {
    ...
    use mscheduler::tasker::consumer::{
        ConsumerEvent, TaskConsumer, TaskConsumerConfig, TaskConsumerFunc,
    };
    ...
    pub async fn test() {
        let collection: Collection<Task<i32, i32>> =
            init_collection_for_test("test_find_task").await;
        let collection2: Collection<Task<i32, i32>> =
            init_collection_for_test("test_find_task").await;
        // consumer
        let worker_id1 = "aaa";
        ...
        let wait4occupy_event_handler = spawn_check_handler(
            task_consumer.clone(),
            |event| match event {
        ...
        // check if ok
        assert!(wait4occupy_event_handler
            .await
            .expect("failed to wait 4 event")
            .is_some());
    }
}
